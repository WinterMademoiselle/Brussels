...
//#endif
	...
	_action_Now = ACTION::RUN;
	_action_Old = ACTION::MAX;
...
{
	if (_action_Now == ACTION::RUN)
	{
		AnimMng::ChangeAnim(delta, "run");
	}
...
void Player::Anim_Registration(Sprite* delta)
{
	...
	/*std::string PlAnim[5] = {"idle","run","duck","shoot-up","jump"};
	std::string plistStr;
	int startNum[5] = { 1,1,0,0,1 };
	int endNum[5] = { 4,10,0,0,6 };
	float actTime[5] = { 0.1,0.08,1.0,1.0,0.05 };
	

	for (int i = 0; i < 1; i++)
	{
		plistStr = "image/Sprites/player/player-" + PlAnim[i] + "/" + PlAnim[i] + "-big.plist";
		AnimMng::addAnimationCache(plistStr, "player-idle-%d-big.png", PlAnim[i], startNum[i], endNum[i], false, actTime[i]);
	}*/
	

	//std::string plistImageStr = "player-" + PlAnim[0] + "-%d-big.png";

	/*for (int i = 0; i <= 23;)
	{
		AnimMng::addAnimationCache(plistStr, image[1][i], PlAnim[0], 1, 4, false, (float)0.1);
		i++;
	}*/
...
{
	// 情報の辿り方(Director->GameScene->PlayerLayer->PlSprite)
